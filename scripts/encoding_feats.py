...
predparser.add_argument('pca_fit_batch', type=int, help='The size of the image batch to fit the PCA to') # Standard is 1000
predparser.add_argument('n_comps', type=int, help='The fixed number of principal components to extract') # Standard is 600
    ...
    while True:  # Keep trying until successful
        try:
            ...
            for i, d in tqdm(enumerate(dataloader), total=len(dataloader)):
                ...
                print(f'AlexNet layer: {cnn_layer}, Mean: {ft.mean()}, Std: {ft.std()}, Min: {ft.min()}, Max: {ft.max()}')
...
def fit_pca(feature_extractor, dataloader, pca_save_path=None, fixed_n_comps:Optional[int]=None, train_batch:int=None):
...
if pca is not None:
    ...
    features_algo = extract_features(feature_extractor, full_dataloader, pca, args.cnn_layer)
else:
    print("PCA fitting failed. Unable to apply PCA.")

np.savez(f'/home/rfpred/data/custom_files/visfeats/cnn_featmaps/featmaps/featmaps_lay{this_layer}.npz', *features_algo)
