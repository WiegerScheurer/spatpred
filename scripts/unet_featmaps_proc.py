...
# DEPPPPPRRRREEECATTTEDDDDDDD DOESN'T WORK!!!! well
import os
...
#     # Reshape the layer to 2D matrix where each row is a flattened image
#     reshaped_layer = np.array(layer).reshape(len(layer), -1)

#     # Run PCA to determine the number of components needed
#     pca = PCA(n_components=0.95)
#     transformed_layer = pca.fit_transform(reshaped_layer)

#     print(f'\tNumber of components needed to explain 95% of the variance: {pca.n_components_}')

#     # Reshape the transformed layer back to original shape but with reduced dimension
#     transformed_layer = transformed_layer.reshape(len(layer), -1, pca.n_components_)

#     # Store the transformed layer
#     transformed_layers[layer_name] = transformed_layer

# # Save the transformed layers
# for layer in range(1, 5):
#     layer_data = np.array(transformed_layers[f'layer_{layer}'])
#     n_imgs, _ = layer_data.shape
#     np.save(f'/home/rfpred/data/custom_files/subj01/pred/featmaps/tests/{scale}_unet_gt_feats_{layer}', layer_data)
# print("All is well")
