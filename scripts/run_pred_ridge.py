...
Xpred = NSP.stimuli.unpred_feats(cnn_type=which_cnn, content=True, style=False, ssim=False, pixel_loss=False, 
                                 L1=True, MSE=False, verbose=True, outlier_sd_bound=5, subject=subject)
...
for layer in range(0,Xpred.shape[1]): # Loop over the layers of the alexnet
    cordict, coords = NSP.analyse.load_regresults(subject, prf_dict, roi_masks, f'{which_cnn}_unpred', f'{str(layer)}', plot_on_viscortex=False, plot_result='r', verbose=False)
    if layer == 0:
...
all_betas_voxroi[:,:Xpred.shape[1]] = stats.zscore(all_betas_voxroi[:,:Xpred.shape[1]], axis=0)
...
df[f'{which_cnn} layer'] = max_indices
...
df_prop = (df.groupby('ROI')[f'{which_cnn} layer']
...
# Save the plot
plt.savefig(f'{NSP.own_datapath}/{subject}/brainstats/{which_cnn}_unpred_layassign.png')
